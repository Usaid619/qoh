...
const Styles = () => {
 ...
 text-center">Timeless classics to modern trends - All in one place!</span>
        ...
        <div className="flex items-center justify-center relative w-full px-10">
          ...
          <button
            onClick={handlePrev}
            className={`absolute left-0 z-10 top-1/2 -translate-y-1/2 p-2 cursor-pointer flex items-center justify-center transition-opacity duration-300 ${index === 0 ? 'opacity-0 pointer-events-none' : 'opacity-100'
              }`}
            disabled={index === 0}
          >
          ...
          <button
            onClick={handleNext}
            className={`absolute right-0 z-10 top-1/2 -translate-y-1/2 p-2 cursor-pointer flex items-center justify-center transition-opacity duration-300 ${index === maxIndex ? 'opacity-0 pointer-events-none' : 'opacity-100'
              }`}
            ...
            >
              {categoryList.map((category) => (
                <div
                  ...
                  className={`[width:calc((100%-10.5rem)/4)] flex-shrink-0 cursor-pointer h-full flex flex-col justify-center items-center rounded-md border border-gray-300 py-3 px-5 shadow-lg transition-colors duration-300 hover:bg-black hover:text-white ${activeButton === category?.name
